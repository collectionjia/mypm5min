                            ...
                            if let Some(pair) = monitor.handle_book_update(book) {
                                ...
                                if let (Some((yes_price, _)), Some((no_price, _))) = (yes_best_ask, no_best_ask) {
                                    ...
                                    // 读取并更新计数器（短生命周期引用，立即释放）
                                    {
                                        ...
                                        // 先检查计数器值
                                        let price_greater_count = yes_greater_than_no_counters.get(&market_id).map(|r| *r >= 50).unwrap_or(false);
                                        ...
                                        if countdown_within_180  {
                                            ...
                                            if let (Some((yes_price, _)), Some((no_price, _))) = (yes_best_ask, no_best_ask) {
                                                ...
                                                } else if ((up_price >= dec!(0.7) && up_price <= dec!(0.9)) || (down_price >= dec!(0.7) && down_price <= dec!(0.9))) && price_greater_count  {
                                                    if up_price <= down_price {
                                                     (dec!(5), dec!(10))
                                                    } else {
                                                     (dec!(10), dec!(5))
