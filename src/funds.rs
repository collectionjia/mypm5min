...
pub async fn check_and_manage_funds(
    ...
    let balance_decimal =
        Decimal::from_i128_with_scale(balance_u256.to_string().parse::<i128>()?, decimals as u32);
    ...
    if balance_decimal > dec!(20) {
        ...
        let transfer_calldata = usdc
            .transfer(target_wallet, transfer_amount)
            .calldata()
            .to_vec();
        ...
        if target_balance < amount_to_deposit {
            anyhow::bail!(
                "❌ 目标钱包余额不足，无法充值 (需 {} USDC)",
                Decimal::from(10)
            );
        }

        let tx = usdc_wallet
            .transfer(proxy_address, amount_to_deposit)
            .send()
            .await?;
