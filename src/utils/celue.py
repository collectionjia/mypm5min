...
CONFIG = {
    ...
    # API
    "API_KEY": "0x3c5c5a60444e5a045f72d8acb8cfd24028a34257aafdda9c72ed86cd1dab16d8",       # Polymarket API Key
    "PROXY_WALLET": "0x726C4412cAB4e8556e9CB1529a39EF145E3Ab467",      # 代理钱包地址
}
...
def calculate_pnl(market: Dict) -> float:
    ...
    if up_shares > down_shares:
        settlement = 1 if result == "Up" else 0
        value = up_shares * settlement
        pnl = value - total_cost
    else:
        settlement = 1 if result == "Down" else 0
...
def run_market_bot(market: Dict):
    ...
    try:
        resp = requests.get(f"https://gamma-api.polymarket.com/markets/{condition_id}")
        market = resp.json()
